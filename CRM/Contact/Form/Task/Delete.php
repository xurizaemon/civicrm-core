...
class CRM_Contact_Form_Task_Delete extends CRM_Contact_Form_Task {
  ...
  public function preProcess() {
    ...
    if ($this->_restore && !CRM_Core_Permission::check('access deleted contacts')) {
      CRM_Utils_System::permissionDenied(ts('You do not have permission to access this contact.'));
    }
    elseif (!CRM_Core_Permission::check('delete contacts')) {
      CRM_Utils_System::permissionDenied(ts('You do not have permission to delete this contact.'));
    }
    ...
    if ($cid) {
      if (!CRM_Contact_BAO_Contact_Permission::allow($cid, CRM_Core_Permission::EDIT)) {
        CRM_Utils_System::permissionDenied(ts('You do not have permission to delete this contact. Note: you can delete contacts if you can edit them.'));
      }
      elseif (CRM_Contact_BAO_Contact::checkDomainContact($cid)) {
        CRM_Utils_System::permissionDenied(ts('This contact is a special one for the contact information associated with the CiviCRM installation for this domain. No one is allowed to delete it because the information is used for special system purposes.'));
      }
